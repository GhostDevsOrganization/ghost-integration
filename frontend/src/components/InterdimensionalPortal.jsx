...
const Kasportal = () => {
  ...
  // Allow portal access even if no wallet is installed
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black">
      {/* Network stats ticker removed */}
      {/* <NetworkStatsTicker kasPrice={kasPrice} setKasPrice={setKasPrice} /> */}
      ...
      {connected ? (
        <>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {/* Energy particles flowing to portal - REMOVED for less movement */}
          ...
          {/* Applied protocol-panel-responsive */}
          <div className={`protocol-panel-responsive absolute right-0 top-0 h-full bg-green-900/80 backdrop-blur-md border-l border-green-700 p-6 
            ...
            {activeProtocol && (
              <div className="h-full flex flex-col">
                ...
                <div className="flex-1 bg-black rounded-lg p-4 text-green-200 overflow-y-auto">
                  {/* Swap Panel */}
                  {/* Swap panel removed: now handled as fullscreen overlay */}
                  
                  {/* Send Panel - Integrating sendKaspa functionality */}
                  {activeProtocol === 'send' && (
                    <div className="p-4 bg-black/40 rounded-lg">
                      ...
                      <div className="mb-4">
                        ...
                        <input 
                          type="text" 
                          className="w-full bg-black/40 border border-green-700 rounded p-2 text-white text-xs font-mono overflow-hidden" 
                      ...
                      <div className="mb-4">
                        ...
                        <input 
                          type="text" 
                          className="w-full bg-black/40 border border-green-700 rounded p-2 text-white" 
                          placeholder="0.0"
                          value={sendAmount}
                      ...
                      <div className="mb-4">
                        ...
                        <div className="w-full bg-black/40 border border-green-700 rounded p-2 text-white">
                          {(balance.total / 100000000).toFixed(8)} KAS
                        </div>
                      </div>
                      <button 
                        className={`w-full bg-green-600 hover:bg-green-500 text-white py-2 rounded-lg transition-colors mb-2 ${isSendingKas ? 'opacity-50 cursor-not-allowed' : ''}`}
                        onClick={handleSendKas}
                      ...
                      <div className="mt-6 border-t border-green-700 pt-4">
                        ...
                        <button 
                          className={`w-full bg-green-600 hover:bg-green-500 text-white py-2 rounded-lg transition-colors ${isSendingToken ? 'opacity-50 cursor-not-allowed' : ''}`}
                          onClick={handleTransferKRC20}
                          disabled={isSendingToken}
                        >
                          {isSendingToken ? 'Sending...' : 'Send KRC-20 Token'}
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {activeProtocol === 'privacy' && (
                    <div className="p-4 bg-black/40 rounded-lg">
                      ...
                      <div className="mb-4">
                        ...
                        <input 
                          type="text" 
                          className="w-full bg-black/40 border border-green-700 rounded p-2 text-white font-mono text-xs" 
                          placeholder="kaspa:address..."
                          value={recipientAddress}
                          onChange={(e) => setRecipientAddress(e.target.value)}
                        />
                      ...
                      <button 
                        className={`w-full bg-green-600 hover:bg-green-500 text-white py-2 rounded-lg transition-colors ${isSendingKas ? 'opacity-50 cursor-not-allowed' : ''}`}
                        onClick={handleSendKas} // Uses the same handler as regular send
                        disabled={isSendingKas}
                      >
                        {isSendingKas ? 'Sending...' : 'Send Private Transaction'}
                      </button>
                  ...
                  {activeProtocol === 'tokens' && (
                    <div className="space-y-6">
                      ...
                      <div className="p-4 bg-black/40 rounded-lg">
                        ...
                        <button 
                          className={`w-full bg-green-600 hover:bg-green-500 text-white py-2 rounded-lg transition-colors ${isDeployingToken ? 'opacity-50 cursor-not-allowed' : ''}`}
                          onClick={handleDeployKRC20}
                      ...
                      <div className="p-4 bg-black/40 rounded-lg">
                        ...
                        <button 
                          className={`w-full bg-green-600 hover:bg-green-500 text-white py-2 rounded-lg transition-colors ${isMintingToken ? 'opacity-50 cursor-not-allowed' : ''}`}
                          onClick={handleMintKRC20}
                  ...
                  {/* Analytics Panel */}
                  {/* Removed embedded Analytics panel for full-screen overlay */}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="absolute top-20 left-0 m-6 z-40 flex items-center space-x-4">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none w-full bg-green-900/80 backdrop-blur-md border border-green-700 text-white py-2 pl-3 pr-8 rounded-lg leading-tight focus:outline-none focus:bg-green-800 focus:border-green-500 text-sm"
              >
          ...
          <div className="absolute bottom-0 w-full px-6 py-4">
             {/* Applied bottom-info-responsive */}
            <div className="bottom-info-responsive bg-black/70 backdrop-blur-sm border border-green-800 rounded-lg p-4 flex text-sm">
              {/* Address and Copy Button */}
              <div className="flex items-center space-x-2">
                <div className="text-white">
                  Address:
                  <span className="text-green-400 font-mono ml-1">{address.substring(0, 10)}...{address.substring(address.length - 6)}</span>
                </div>
              ...
              <div className="text-white text-base"> {/* Base size for label */}
                  Balance: <span className="text-green-400 font-bold text-lg ml-1">{(balance.total / 100000000).toFixed(4)} KAS</span> {/* Larger font, bold */}
              </div>

              {/* Network Info */}
              <div className="text-center text-white">
                <Activity className="inline-block mr-2 text-green-300" size={16} />
                Network: <span className="text-green-400">{network.replace('kaspa_', '')}</span>
              </div>

              {/* Status Info (Original) */}
        ...
        <div className="absolute inset-0 flex items-center justify-center z-50">
          <div className="p-8 bg-green-900/70 backdrop-blur-lg rounded-xl max-w-md text-center">
            <h1 className="text-3xl font-bold text-white mb-4">Welcome to <span className="text-green-400">Kasportal</span></h1>
            <p className="text-green-100 mb-6">Connect your Kasware wallet to access the Kaspa ecosystem</p>
            <button 
            ...
            <div className="mt-6 text-green-300 text-sm">
              <p>Access KRC-20 tokens, private transactions, swaps and more</p>
            </div>
          </div>
        </div>
      )}
...
/**
 * AnalyticsChartWrapper: Reads portfolio history from localStorage and passes it to BlackPortfolioChart.
 */
